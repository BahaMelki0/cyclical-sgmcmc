...
class CyclicalSGHMC(Optimizer):
    """
    Cyclical Stochastic Gradient Hamiltonian Monte Carlo (cSGHMC) optimizer.
    
    Implementation of cSGHMC from the paper:
    ...
    def __init__(self, params, init_lr, num_data, num_cycles, total_iterations, exploration_ratio=0.8, 
                 momentum=0.9, temperature=1.0, weight_decay=0, noise_scale=0):
        if init_lr < 0.0:
        ...
        super(CyclicalSGHMC, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if not in_exploration and current_temp > 0:
                    noise = torch.randn_like(p.data) * math.sqrt(2.0 * lr * current_temp * (1.0 - momentum) - 2.0 * lr * lr * noise_scale)
                    buf.add_(noise)
