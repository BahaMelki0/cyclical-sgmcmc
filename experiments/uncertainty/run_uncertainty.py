...
from utils.data_utils import get_cifar10_dataloaders, get_cifar100_dataloaders, get_svhn_dataloaders
from utils.metrics import accuracy, negative_log_likelihood, brier_score, expected_calibration_error
from utils.visualization import plot_uncertainty_histogram
...
def evaluate_uncertainty(models, test_loader, device, num_classes):
    """
    ...
    # Calculate metrics
    nll = negative_log_likelihood(torch.log(all_outputs), all_targets)
    bs = brier_score(all_outputs, all_targets, num_classes)
...
def main():
    parser = argparse.ArgumentParser(description='Run uncertainty estimation experiments')
    parser.add_argument('--task', type=str, default='svhn_to_cifar10', choices=['svhn_to_cifar10', 'cifar10_to_svhn'], help='Transfer task')
    ...
    parser.add_argument('--results_dir', type=str, default='../../results/uncertainty', help='Directory to save results')
    ...
    # Load data
    if args.task == 'svhn_to_cifar10':
        # Train on SVHN, test on CIFAR-10
        train_loader, in_test_loader = get_svhn_dataloaders(args.batch_size)
        out_test_loader, _ = get_cifar10_dataloaders(args.batch_size)
        num_classes = 10
        exploration_ratio = 0.8
    else:  # cifar10_to_svhn
        # Train on CIFAR-10, test on SVHN
        train_loader, in_test_loader = get_cifar10_dataloaders(args.batch_size)
        out_test_loader, _ = get_svhn_dataloaders(args.batch_size)
        num_classes = 10
        exploration_ratio = 0.8
    ...
    for method in args.methods:
        ...
        if method == 'sgd':
            ...
            for epoch in range(args.epochs):
                train_loss, train_acc = train_epoch(model, optimizer, train_loader, device, epoch, args)
                test_loss, test_acc = test(model, in_test_loader, device)
                scheduler.step()
            ...
            # Collect samples
            samples, test_losses, test_accs = collect_samples(model, optimizer, train_loader, in_test_loader, device, 150, 50, args.num_samples, args)
            ...
            # Collect samples
            samples, test_losses, test_accs = collect_samples(model, optimizer, train_loader, in_test_loader, device, 0, args.epochs, 3 * args.num_cycles, args)
        ...
        # Evaluate uncertainty on in-distribution data
        in_uncertainty = evaluate_uncertainty(samples, in_test_loader, device, num_classes)
        
        # Evaluate uncertainty on out-of-distribution data
        out_uncertainty = evaluate_uncertainty(samples, out_test_loader, device, num_classes)
        ...
        results[method] = {
            ...
            'test_accs': test_accs,
            'in_uncertainty': in_uncertainty,
            'out_uncertainty': out_uncertainty
        }
        
        # Plot uncertainty histograms
        plot_uncertainty_histogram(
            in_uncertainty['confidences'],
            in_uncertainty['correct'],
            save_path=os.path.join(args.results_dir, f'{args.task}_{method}_in_uncertainty.png')
        )
        
        plot_uncertainty_histogram(
            out_uncertainty['confidences'],
            out_uncertainty['correct'],
            save_path=os.path.join(args.results_dir, f'{args.task}_{method}_out_uncertainty.png')
        )
        
        print(f"{method} in-distribution NLL: {in_uncertainty['nll']:.4f}, Brier score: {in_uncertainty['brier_score']:.4f}, ECE: {in_uncertainty['ece']:.4f}")
        print(f"{method} out-of-distribution NLL: {out_uncertainty['nll']:.4f}, Brier score: {out_uncertainty['brier_score']:.4f}, ECE: {out_uncertainty['ece']:.4f}")
    
    # Save results
    torch.save(results, os.path.join(args.results_dir, f'{args.task}_results.pt'))
    
    # Print summary of results
    print("\nSummary of in-distribution results:")
    for method in args.methods:
        print(f"{method}: NLL={results[method]['in_uncertainty']['nll']:.4f}, Brier={results[method]['in_uncertainty']['brier_score']:.4f}, ECE={results[method]['in_uncertainty']['ece']:.4f}")
    
    print("\nSummary of out-of-distribution results:")
    for method in args.methods:
        print(f"{method}: NLL={results[method]['out_uncertainty']['nll']:.4f}, Brier={results[method]['out_uncertainty']['brier_score']:.4f}, ECE={results[method]['out_uncertainty']['ece']:.4f}")
